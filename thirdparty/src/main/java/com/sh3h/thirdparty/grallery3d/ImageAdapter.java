package com.sh3h.thirdparty.grallery3d;
...
public class ImageAdapter extends BaseAdapter {
    ...
    private static final int SD = 1001;
    int mtype;
...
//    public String[] titles = {"美图01", "美图02", "美图03", "美图04", "美图05", "美图06", "美图07"};
    //  Integer[] mImageIndies;

    //    String[] titles;
    public ImageAdapter(Context c, List<Object> list, int type) {
        this.mContext = c;
        mList = list;
        mtype = type;
...
//            Integer id = (Integer) map.get("image");
            // 获取原始图片
            if (mtype == SD) {
                originalImage = BitmapFactory.decodeFile(object.toString());
            } else {
                originalImage = BitmapFactory.decodeResource(mContext.getResources(), Integer.parseInt(object.toString()));
            }

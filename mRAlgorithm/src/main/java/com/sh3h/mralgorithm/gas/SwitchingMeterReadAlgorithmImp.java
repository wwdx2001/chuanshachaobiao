...
public class SwitchingMeterReadAlgorithmImp extends ReadAlgorithm {
    public SwitchingMeterReadAlgorithmImp() {

    }

    @Override
    public int getCode() {
        return IReadAlgorithm.ALGORITHM_SWITCHTINGMETER;
    }
    ...
    public ReadingResult calculate(ReadingParameter param) {
        ...
        int xinbiaoxfl = param.get_bencicm() - param.get_xinbiaodm();
        int jiubiaoxfl = 0;
        int buyalc = 0;
        int bushoul = 0;
        int xiaofeizl = 0;

        if (param.getHuanBiao() == 1) {
            jiubiaoxfl = param.get_jiubiaocm() - param.get_shangcicm();
        }
    ...
    private ReadingResult estimate(ReadingParameter param) {
        ...
        param.setXiaoFeiZL(xiaofeizl);
        param.setKongZhiRLF(calKongZhiRLF(param));
        param.set_bencicm(xinbiaoxfl + param.get_xinbiaodm());
        param.set_feizhengcjl(0);

...
final class LoggerPrinter implements Printer {
  ...
  private final List<LoggerMessage> loggerMessageList = Collections.synchronizedList(new ArrayList<LoggerMessage>());
  ...
  private void logChunk(int logType, String tag, String chunk) {
    ...
    if ((finalTag != null)
            ...
            && (chunk.length() > 0)
            && (mThread != null)) {
      synchronized (loggerMessageList) {
        loggerMessageList.add(new LoggerMessage(finalTag, chunk));
      }
  ...
  private void print2File(String tag, String message) {
    try {
      if (filePath == null) {
        return;
      }
      ...
      Writer writer = new BufferedWriter(new FileWriter(file.getAbsolutePath(), true), 2048);
          ...
          while (true) {
            synchronized (loggerMessageList) {
              if (loggerMessageList.size() > 0) {
                LoggerMessage loggerMessage = loggerMessageList.remove(0);
                if ((loggerMessage != null)
                        && (loggerMessage.getTag() != null)
                        && (loggerMessage.getMessage() != null)) {
                  print2File(loggerMessage.getTag(), loggerMessage.getMessage());
                }
              } else {
                break;
              }
  ...
  private class LoggerMessage {
    ...
    public LoggerMessage(String tag, String message) {
      this.tag = tag;

...
//获取sd卡路径
            ...
            String sh3h = "sh3h/meterreading/data/";//数据库所在目录
            String backupData = "data/";//数据库所在目录
            String logPath = "sh3h/logs";
            File logFileDir = new File(rootPath, logPath);
            if (!logFileDir.exists()) {
                logFileDir.mkdirs();
            }
            LogUtils.getConfig().setDir(logFileDir);
            LogUtils.getConfig().setSaveDays(30);
            //判断目录是否存在，不存在则创建该目录

            File dirFile = new File(rootPath, sh3h);
            File backupDataFile = new File(rootPath, backupData);
            if (!backupDataFile.exists()) {
                backupDataFile.mkdirs();
            }
            if (!dirFile.exists())
                dirFile.mkdirs();

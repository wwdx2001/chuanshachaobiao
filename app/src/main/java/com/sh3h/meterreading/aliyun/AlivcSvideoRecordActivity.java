...
public class AlivcSvideoRecordActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mInputParam != null) {
            ...
            videoRecordView.setVideoOutputPath(videoOutputPath);
            //配置录制recorder
    ...
    private void initAssetPath() {
        initAssetPath = new AlivcSvideoRecordActivity.AssetPathInitTask(this).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
    }
    ...
    private void copyAssets() {
        new Handler().postDelayed(new Runnable() {
            @Override
            public void run() {
                copyAssetsTask = new AlivcSvideoRecordActivity.CopyAssetsTask(AlivcSvideoRecordActivity.this).executeOnExecutor(
...
//                progressBar.setMessage(activity.getResources().getString(R.string.aliyun_res_copy));
    ...
    private void getData() {
        ...
        videoOutputPath = intent.getStringExtra(AlivcRecordInputParam.INTENT_KEY_VIDEO_OUTPUT_PATH);
...
//                FixedToastUtils.show(AlivcSvideoRecordActivity.this, "视频录制完成");
                setResult(Activity.RESULT_OK, new Intent().putExtra("url", path));
                finish();
...
//                EditorActivity.startEdit(AlivcSvideoRecordActivity.this, param);
    ...
    public static void startRecord(Activity context, AlivcRecordInputParam recordInputParam) {
        ...
        context.startActivityForResult(intent, SNAP_VIDEO);
    }

    public static void startRecord(Fragment context, AlivcRecordInputParam recordInputParam) {
        Intent intent = new Intent(context.getActivity(), AlivcSvideoRecordActivity.class);
        intent.putExtra(AlivcRecordInputParam.INTENT_KEY_RESOLUTION_MODE, recordInputParam.getResolutionMode());
        intent.putExtra(AlivcRecordInputParam.INTENT_KEY_MAX_DURATION, recordInputParam.getMaxDuration());
        intent.putExtra(AlivcRecordInputParam.INTENT_KEY_MIN_DURATION, recordInputParam.getMinDuration());
        intent.putExtra(AlivcRecordInputParam.INTENT_KEY_RATION_MODE, recordInputParam.getRatioMode());
